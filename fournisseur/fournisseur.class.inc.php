<?php
	class Fournisseur
	{
		...
		Public function __construct(string $idfournisseur='', string $nomfournisseur='', string $adressefournisseur='', string $telephonefournisseur='', string $emailfournisseur='', string $idcommune='', string $prefixefournisseur='', string $idtarif='', string $idsociete='', string $idpaiement='', string $codefournisseur='')
		{
		...
		{
			$stmt = $pdo->prepare('SELECT * FROM fournisseur INNER JOIN commune ON commune.idcommune = fournisseur.idcommune INNER JOIN tarif ON tarif.idtarif = fournisseur.idtarif INNER JOIN societe ON societe.idsociete = fournisseur.idsociete INNER JOIN paiement ON paiement.idpaiement = fournisseur.idpaiement WHERE supfournisseur = 0 AND idfournisseur = :idfournisseur');
		...
		{
			$stmt = $pdo->prepare('UPDATE fournisseur SET nomfournisseur = :nomfournisseur, adressefournisseur = :adressefournisseur, telephonefournisseur = :telephonefournisseur, emailfournisseur = :emailfournisseur, idcommune = :idcommune, prefixefournisseur = :prefixefournisseur, idtarif = :idtarif, idsociete = :idsociete, idpaiement = :idpaiement, codefournisseur = :codefournisseur WHERE idfournisseur = :idfournisseur');
			$stmt->bindValue(':idfournisseur',$this->idfournisseur, PDO::PARAM_STR);
		...
		{
			$stmt = $pdo->prepare("INSERT INTO fournisseur (idfournisseur, nomfournisseur, adressefournisseur, telephonefournisseur, emailfournisseur, idcommune, prefixefournisseur, idtarif, idsociete, idpaiement, codefournisseur, supfournisseur) VALUES (NULL, :nomfournisseur, :adressefournisseur, :telephonefournisseur, :emailfournisseur, :idcommune, :prefixefournisseur, :idtarif, :idsociete, :idpaiement, :codefournisseur, 0);");
			 // var_dump($stmt);
    //         var_dump($this);
    //         die(); 
			$stmt->bindValue(':nomfournisseur',$this->nomfournisseur, PDO::PARAM_STR);
