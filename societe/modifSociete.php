...
<?php
	...
	if ($_POST['action'] == 'modifier')
	{
		...
		$ob->GetByID($conn);
		$action = 'modifier';
			echo "<p class='size-18 text-center'><b>Modifier la catégorie ",$ob->Getlibsociete(), "</p><br>";
	}
	else
	{
		$ob = new Societe();
...
<form method='POST' action='traitementSociete.php'>
  <div class="row">
	<input name='idsociete' type='hidden' value="<?php echo $ob->Getidsociete(); ?>"/>
		<div class="col-5">Lib societe:
			<input name='libsociete' type='text' value="<?php echo $ob->Getlibsociete(); ?>"/>
		</div>
  </div>
	<input name='action' type='hidden' value="<?php echo $action; ?>"/>
	<a class='button bg_dark-radius_5' href="societe.vue.php">Retour</a>
